...
Supporting directories: `prompts/` (versioned markdown prompt files — `reviewer-v1..v13.md`, `verifier-v1..v3.md`, `verifier-meta-v1.md`), `evals/` (offline eval harness), `openspec/` (source of truth), `research/` (design corpus), `guides/` (human-facing narrative), `docs/` (Ansh's personal notes).
...
| `calibration.ts` | Empirical calibration (feature #29): build a keep-rate table from run-log history and pre-suppress persistently-low-keep-rate finding shapes before the model. |
| `ci.ts` | CI-output ingestion (feature #16): parse the repo's existing SARIF/eslint/tsc output, filter to touched files, inject as ground truth for the verifier. |
| `ctags.ts` | ctags-lite symbol index over PR-head files (feature: ranked symbol defs rendered as context); capped, fail-soft. |
| `deps.ts` | Supply-chain pass (feature #22): scan lockfile diffs for new dependencies + install scripts, optional OSV.dev audit; deterministic findings. |
| `feedback.ts` | Feedback observability (feature #12): reaction tallies + thread resolution on the bot's own prior comments, classified accepted/disputed/unresolved and written to the run log. |
| `history.ts` | Git-history context (feature #20): churn + author-count per changed file via the API, rendered as `{{HISTORY_CONTEXT}}`; also the churn escalation signal. |
| `reflect.ts` | Bounded reflection (feature #27): one extra critique pass over critical/high `keep` verdicts; demotes (never drops) findings whose evidence doesn't establish the claim. |
| `relatedtests.ts` | Test-context track (feature #17): sibling-test discovery for changed files → `{{RELATED_TESTS}}` block. |
| `reply.ts` | Conversational replies to the bot's own inline comments (feature #32, Worker/App path). |
| `repomap.ts` | Ranked repo-map priming: a compact tree of the repo's top files rendered into the prompt. |
| `selfconsistency.ts` | Self-consistency voting (feature #15): re-run high-stakes findings; disagreement demotes, never drops. |
| `sinkpack.ts` | Dangerous-sink SAST-lite rule pack (feature #21): per-language regex pack over added lines rendered as pre-flagged evidence lines. |
| `suggestions.ts` | Learned-rule suggestion queue output (feature #31 side-channel): a suggestions file mined from feedback, human-applied, never auto-applied. |
| `symbols.ts` | TS virtual language service diagnostics (feature #33): `tsc`-class diagnostics over PR-head files as zero-hallucination findings. |
| `injection` (in `guardrail.ts`/`prompt.ts`) | Prompt-injection self-defense (feature #23): scan attacker-reachable prompt text for injection markers/zero-width Unicode; strip + surface a notice. |
...
20. **Publish** (`publish.ts`, `summary.ts`) — unless `dryRun`: exactly ONE
    ...
    absent). The batched review is submitted as `REQUEST_CHANGES` instead of
    `COMMENT` when the blocking tier is configured
    (`EngineConfig.blockingSeverities` or `.aireview.toml`
    `blocking_severities`) and at least one published finding meets the floor —
    a deterministic, opt-in separation of blocking vs advisory comments.
...
| **`EngineConfig`** | provider/model/baseUrl/apiKey, `escalationModel`, `minSeverity`, `blockingSeverities` (blocking tier → REQUEST_CHANGES), `event`, caps, feature flags (`agentic`, `verify`, `escalation`, `rag`, `prIntent`, `crossFileCallers`, …), paths (`ledgerPath`, `runLogPath`, `configPath`, `promptPath`), `dryRun`, `botIdentity` | The full run configuration; most features default OFF until proven. |
