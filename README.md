    ...
    steps:
      - uses: anshace/loupe@v1
        with:
          ...
          # blocking-severities: critical,high  # submit as REQUEST_CHANGES when a finding is this severe
          # verify: true               # second-pass verifier that kills unsupported findings (fail-open)
          # agentic: true              # capped read-only grep/read tool loop for the model
          # cross-file-callers: true   # force-inject call sites of changed exported signatures
...
| `web/` | **Website + web workspace** (Next.js): product landing page, auth, repo/PR scanning with the shared engine, report history. See `web/README.md` |

## Website (web workspace)

`web/` is a full-stack Next.js 16 app around the same engine: a redesigned
product site plus a working workspace that scans real GitHub repos/PRs
(deterministic secret scan + static checks, optional opt-in AI review) with
PostgreSQL persistence, Supabase or standalone email/password auth, and a
searchable report history. It reuses the engine's diff parser and secret
scanner directly.

```bash
cd web
npm install
cp .env.example .env   # set DATABASE_URL (any PostgreSQL) and APP_URL
npx drizzle-kit push
npm run dev            # http://localhost:3000
```

Deploying (Vercel + Supabase) and full feature/limit notes: `web/README.md`
and the in-app `/docs` page.
