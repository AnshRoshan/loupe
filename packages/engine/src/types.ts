...
export interface EngineConfig {
  ...
  /**
   * Blocking vs advisory tier separation. When set and at least one published
   * finding (inline or summary-mention) is at least as severe as the LEAST
   * severe listed severity, the batched review is submitted with event
   * REQUEST_CHANGES (blocking tier) instead of the default COMMENT (advisory).
   * Opt-in and deterministic — the model never chooses what blocks. Absent →
   * always COMMENT. Only meaningful on PRs from the same repo (GitHub rejects
   * REQUEST_CHANGES from forks without write access; the post then fails soft
   * like any other publish error).
   */
...
export interface ReviewPayload {
  body: string;
  /** "REQUEST_CHANGES" only when the blocking tier fired (EngineConfig.blockingSeverities). */
