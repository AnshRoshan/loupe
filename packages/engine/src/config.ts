...
export interface RepoConfig {
  ...
  /**
   * Blocking tier: when a published finding is at least as severe as the LEAST
   * severe entry here, the review is submitted as REQUEST_CHANGES instead of
   * COMMENT. Empty → advisory always (COMMENT).
   */
...
export function parseAireviewToml(text: string): ParsedRepoConfig {
  ...
  // Blocking tier (EngineConfig.blockingSeverities): severities whose presence
  // in the published findings submits the review as REQUEST_CHANGES.
  if ("blocking_severities" in values) {
    const v = values.blocking_severities;
    if (Array.isArray(v) && v.every((s) => (SEVERITIES as readonly string[]).includes(s))) {
      config.blockingSeverities = v as Severity[];
    } else {
      problems.push(`"blocking_severities" must be an array of ${SEVERITIES.join("/")}`);
    }
  }
  if ("ignore" in values) {
