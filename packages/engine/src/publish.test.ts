...
describe("buildReviewPayload blocking tier", () => {
  const medium: Finding = { ...fileLevel };
  const highInline: Finding = { ...inline, severity: "high" };
  const nitInline: Finding = { ...inline, severity: "nit" };

  it("defaults to COMMENT when blockingSeverities is undefined (advisory)", () => {
    const payload = buildReviewPayload("s", [at(highInline, "line")]);
    expect(payload.event).toBe("COMMENT");
  });

  it("defaults to COMMENT when blockingSeverities is empty", () => {
    const payload = buildReviewPayload("s", [at(highInline, "line")], {}, []);
    expect(payload.event).toBe("COMMENT");
  });

  it("escalates to REQUEST_CHANGES when a published finding meets the floor", () => {
    const payload = buildReviewPayload("s", [at(highInline, "line")], {}, ["high"]);
    expect(payload.event).toBe("REQUEST_CHANGES");
  });

  it("meets the floor when the finding is MORE severe than the listed minimum", () => {
    const payload = buildReviewPayload("s", [at({ ...inline, severity: "critical" }, "line")], {}, ["high"]);
    expect(payload.event).toBe("REQUEST_CHANGES");
  });

  it("stays COMMENT when every finding is below the floor", () => {
    const payload = buildReviewPayload("s", [at(nitInline, "line")], {}, ["high"]);
    expect(payload.event).toBe("COMMENT");
  });

  it("counts summary-mention (file-level) published findings toward the tier", () => {
    const payload = buildReviewPayload("s", [at(medium, "file")], {}, ["medium"]);
    expect(payload.event).toBe("REQUEST_CHANGES");
  });

  it("uses the LEAST severe listed severity as the floor", () => {
    const payload = buildReviewPayload("s", [at(medium, "file")], {}, ["critical", "medium"]);
    expect(payload.event).toBe("REQUEST_CHANGES");
  });

  it("stays COMMENT when blocking is requested but nothing was published", () => {
    const payload = buildReviewPayload("s", [], {}, ["critical"]);
    expect(payload.event).toBe("COMMENT");
  });
});
