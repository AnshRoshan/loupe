...
export function buildReviewPayload(
  ...
  // Blocking vs advisory tier: REQUEST_CHANGES only when the caller opted in
  // (EngineConfig.blockingSeverities) AND a published finding (inline comment or
  // summary-mention — anything in `findings`) meets the floor, which is the LEAST
  // severe listed severity. Deterministic — the model never chooses what blocks.
  let event: ReviewPayload["event"] = "COMMENT";
  if (blockingSeverities && blockingSeverities.length > 0 && findings.length > 0) {
    const floor = Math.min(...blockingSeverities.map((s) => severityRank(s)));
    if (findings.some((a) => severityRank(a.finding.severity) >= floor)) {
      event = "REQUEST_CHANGES";
    }
  }
  return { body: summary, event, comments };
}
