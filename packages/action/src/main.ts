...
async function run(): Promise<void> {
  ...
  // ── blocking-severities: comma-separated severity floor(s) for the blocking
  // tier. When a published finding is at least as severe as the LEAST severe
  // entry, the review is submitted as REQUEST_CHANGES instead of COMMENT.
  // Invalid entries are dropped with a warning; an empty result = advisory.
  const blockingRaw = input("blocking-severities")
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);
  const blockingSeverities = blockingRaw.filter((s) => SEVERITIES.includes(s)) as Severity[];
  if (blockingRaw.some((s) => !SEVERITIES.includes(s))) {
    core.warning(`ignoring invalid blocking-severities entries — use: ${SEVERITIES.join(" | ")}`);
  }

  // ── Engine feature flags the .aireview.toml can't easily express per-run and
  // that the engine defaults OFF until proven. Each is opt-in via `with:`; the
  // engine remains the single source of truth for behavior and caps.
  const boolInput = (name: string, envKey?: string): boolean | undefined => {
    const raw = input(name, envKey).toLowerCase();
    if (!raw) return undefined;
    if (raw === "true") return true;
    if (raw === "false") return false;
    core.warning(`ignoring invalid ${name} "${raw}" — use "true" or "false"`);
    return undefined;
  };
  ...
  const engineConfig: EngineConfig = {
    ...
    escalationModel,
    // Blocking vs advisory tier (advisory COMMENT when absent).
    blockingSeverities: blockingSeverities.length > 0 ? blockingSeverities : undefined,
    // Feature flags: verifier pass, agentic tool loop, cross-file caller
    // injection, related-tests context, git-history context, SAST-lite sink
    // pack, walkthrough narrative, and prompt-injection defense. All optional;
    // when unset the engine/repo-config default applies.
    verify: boolInput("verify"),
    agentic: boolInput("agentic"),
    crossFileCallers: boolInput("cross-file-callers"),
    relatedTests: boolInput("related-tests"),
    historyContext: boolInput("history-context"),
    sinkPack: boolInput("sink-pack"),
    walkthrough: boolInput("walkthrough"),
    injectionDefense: boolInput("injection-defense"),
  };
