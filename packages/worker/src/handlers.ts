...
/**
 * Feature-flag parity with the Action adapter, via Worker env vars: VERIFY,
 * AGENTIC, CROSS_FILE_CALLERS ("true"/"false") and BLOCKING_SEVERITIES
 * (comma-separated). Unset → the engine default (all OFF, advisory COMMENT).
 */
function featureFlags(env: WorkerEnv): Partial<EngineConfig> {
  const rec = env as unknown as Record<string, string | undefined>;
  const bool = (key: string): boolean | undefined => {
    const raw = rec[key]?.toLowerCase();
    if (raw === "true") return true;
    if (raw === "false") return false;
    return undefined;
  };
  const blocking = (rec.BLOCKING_SEVERITIES ?? "")
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter((s) => SEVERITIES.includes(s)) as EngineConfig["blockingSeverities"];
  return {
    verify: bool("VERIFY"),
    agentic: bool("AGENTIC"),
    crossFileCallers: bool("CROSS_FILE_CALLERS"),
    ...(blocking && blocking.length > 0 ? { blockingSeverities: blocking } : {}),
  };
}
